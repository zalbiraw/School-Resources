...
int add_delta(double, int, int, vector<vector<T>>*, unordered_map<vector<T>, double>*);
int good_turing(double, int, int, int, vector<vector<T>>*, unordered_map<vector<T>, double>*);
void get_possible_sentences(vector<vector<T>>*, vector<vector<vector<T>>>*, vector<T>);
vector<T> empty_vector;
double add_one_value, add_delta_value, good_turing_value;
...
int main(int argc, char **argv)
{
	if (argc < 4)
	{
		...
		cout << "Required format: P7 text1.txt text2.txt text3.txt" << endl;
	}
	else
	{
		char* train = argv[1], *test = argv[2], *dict = argv[3];
		int n = 2;
		double threshold = 1, delta = 0.0005;
		...
		try
		{
			...
			{
				int print;
				int ao = add_delta(1, (unique * 2), n, &sentences_pool[i], &database);
				int ad = add_delta(delta, (unique * 2), n, &sentences_pool[i], &database);
				int gt = good_turing(threshold, (unique * 2), database[empty_vector], n, &sentences_pool[i], &database);

				if (add_one_value > add_delta_value && add_one_value > good_turing_value)
					print = ao;
				else if (add_delta_value > add_one_value && add_delta_value > good_turing_value)
					print = ad;
				else
					print = gt;

				for (int x = 0; x < sentences_pool[i][print].size(); x++)
					cout << sentences_pool[i][print][x] << " ";
				cout << endl;
			}
		}
...
int add_delta(double delta, int magnitude, int n, vector<vector<T>> *sentences
	, unordered_map<vector<T>, double> *ml)
{
	...
	if (delta == 1)
		add_one_value = best_probability;
	else
		add_delta_value = best_probability;

	return best_sentence;
}

int good_turing(double threshold, int magnitude, int size, int n, vector<vector<T>> *sentences
	, unordered_map<vector<T>, double> *ml)
{
	...
	good_turing_value = best_probability;
	return best_sentence;
}
